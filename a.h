#define l0 5
#define n0 (1LL<<l0)
#define ZU static U//abc.efg.ij.lmno..r.t...x.. $sv
#define ZV static V//ABCD.FGHI....NOPQR.TUVW...
#define AV(n) __attribute((vector_size(1<<n),aligned(1)))
typedef char d2 AV(l0-2),V AV(l0);typedef unsigned u,d1 AV(l0-1),Vu AV(l0);typedef unsigned long long U,U4 AV(4),VU AV(l0),UU AV(l0+1),(*Uf)(U),(*UF)(U,U);
...
#if __arm64
#define z(f) bi(neon_v##f##q_v)  //VF(mu,X2(o(max)(X(a),X(b),48)))#define S(f,i) static AN(f,"mov x16,"#i"\nldr x1,[sp]\nldr x2,[sp,#8]\nsvc #0\nret",Ux,...)
ZU A=1;S(w_,4)S(_w,3)S(f_,5)S(_f,6)S(_l,338)S(_k,1)S(m_,197)AN(ut,"mrs x0,cntvct_el0\nret")VF(gg,Vv={};i(2,asm("fcmla.4s %0,%1,%2,0\nfcmla.4s %0,%1,%2,90":"+w"(X(v)):"w"(X(a)),"w"(X(b))))v)
g(X9,-f^bi(neon_vmull_p64)(x,-1L))X x8={1,2,4,8,16,32,64,128,1,2,4,8,16,32,64,128};U(b_,Xz=z(padd)(x0&x8,x1&x8,48);i(2,z=z(padd)(z,z,48))*(u*)&z,Va)U(B_,0,Va)V(sq,a,Va)VF(A0,a)VF(lu,a)VF(mu,a)VF(mg,a)
#else
#define S(f,i) static AN(f,"mov %rcx,%r10;mov $"#i",%rax;syscall;ret",Ux,...)
ZU A;S(_w,0)S(w_,1)S(f_,2)S(_f,3)S(_l,4)S(_k,60)S(m_,9)AN(ut,"rdtsc;shl $32,%rdx;or %rdx,%rax;ret")g(X9,-f^bi(ia32_pclmulqdq128)((U4){x},~(U4){},0)[0])
AN(_start,"lea 8(%rsp),%rsi;call main")
#define z(f) bi(ia32_##f##256)
U(b_,z(pmovmskb)(a),Va)U(B_,z(movmskps)(a),Ve a)
VF(ag,a+b)VF(lg,z(pminub)(a,b))VF(mg,z(pmaxub)(a,b))VF(ai,(Vu)a+(Vu)b)VF(lu,z(pminud)(a,b))VF(mu,z(pmaxud)(a,b))VF(ae,(Ve)a+(Ve)b)
V(sq,z(sqrtps)(a),Ve a)VF(gg,z(vfmaddsubps)((Ve)a,(Ve)z(pshufd)(b,0xa0),(Ve)z(pshufd)(a,0xb1)*(Ve)z(pshufd)(b,0xf5)))VF(A0,B(I0/16==b/16,z(pshufb)(a,b),z(pshufb)(z(permvarsi)(a,4+I0/4),b)))
#endif
...
#define S0(o,b) Va=b;_N(Vv=O;      v=o(v,s(1,v,b));v=o(v,s(2,v,b));v=o(v,s(4,v,b));v=o(v,s(8,v,b));v=o(v,s(16,v,b));a=o(v,a[31]+Z))  //gie &|+
#define bI(z)    R(1,nx,I(Nx,rc=B_(e(z))))
#define bC(z)    R(1,nx,I(Nx,ru=b_(e(z))))
#define _(z)   e(typeof(e(z))_Z=e(z);_r(x);_Z)
#define _a(z)  e(typeof(e(z))_Z=e(z);_r(a);_Z)
#define __(z)  e(typeof(e(z))_Z=e(z);_r(a);_r(x);_Z)
#define a(z,y) e(typeof(z)a=z;y)
#define b(z)   a(z,(64>a?1LL<<a:0)-1) //((1LL<<(z))-1)
...
#define Zu static u
Zu N=1<<31,L[]={3,0,0,2,2,3};Ui(tb,t(1,i))Ui(tc,t(2,i))Ui(ti,t(3,i))f(l2,64-lb(x-1))g(tn,1==f?7+x>>3:x<<L[f])static float E[]={1,10,100,1e3,1e4,1e5,1e6,1e7,1e8,1e9};
...
g(ig,cc=f;I(nx,Ua=b_(c==O);P(a,l(nx,n0*i+ib(a))))nx)g(qg,nx>ig(f,x))
Ui(g_,in=95<<23;ij=N&i;j>>2|b(29)&m(n,i)-n)Ui(_g,in=95<<23;ij=i&b(29);j?N&i<<2|j+n:0)f(G_,g_(x)|g_(x>>32)<<30)f(_G,_g(x)|_g(x>>30)<<32)f(z4,64/4-lb(x)/4)
...
nS(ns,R(2,n,dn(r,n,(U)s)))Us(xs,ns(sc(s,0),s))U(v,_r(*_);*_=x,U*_,Ux)U(OO,w_(2,"OO\n",3);_k(0))F(k2,K(2,i?x:a))Zc*_P,B=32,*PP=
"+- *    %     &and  |or   <    >     =      ~ .! @at    ?find #take  _drop  ^cut   ,$\n+- *sqr %sqrt &flip |flop <asc >desc =group ~ .! @first ?nub  #count _floor ^order ,$";
...
g(a_,ii=f;t(Tx,$6(Tx,XU,1&xb>>i%32,xc,xu,xu,G_(xU))))f(v_,R(tx,1,*UR=5==tx?_G(x):x))f(_v,_(a_(0,x)))F(a0,ii=0;$4(Tx-1,xc&=254,xc=a,xu=a,xU=a);x)U(f1,_K($(a_(i,x))),Uf $,Ux)U(fr,_K($(a,a_(i,x))),UF $,Ua,Ux)
g(T,ii=x?l2(n0+tn(f,x))-l0-1:0;r(M_(i)+n0,RH=i<<16|f<<24|x<<32))v1(_t,3>Tx?_N(O+(26>O-65&32)):_T(Tx-1,5>Tx?C(Vi,Ex):(Vi)sq(_u*_u+u_*u_)))
...
U(ln,R_(R(Tx,n,dn(r,(U)n<<Lx,x+((int)i<<Lx)))),ii,in,Ux)F(dx,Un=Nx;ij=n%n0;a=dn(a,n-=j,x);i(j,ac=sx[n+i])a+j)F(V_,__(R_(R(Ta,na+nx,dx(dn(r,x(a,Nx),a),x)))))
g(sS,in=f?nx:0;ij;n(Ua=xU;n+=j=na)_R(a(xx,Ta),n,Ua=r;n(if(a=dx(a,xU),f)*sa++=f)))F(_V,__(K(na,ln(au,(i+1<na?((u*)a)[i+1]:nx)-au,x))))Ui(c1,v_(tc(i)))g(vc,V_(x,c1(f)))
nS(i$,48>*s?-i$(n-1,s+1):r(0,i(n,r=si%16+10*r)))nS(e$,P(48>*s,N|e$(n-1,s+1))ii=nsc(n,s,101);ue(n>i?i$(i,s)*E[s[i+1]%16]:n>(i=nsc(n,s,46))?(n-=i+1,(i$(i,s)*E[n]+i$(n,s+i+1))/E[n]):i$(n,s)))
...
f($a,x=$3(tx-3,$i(ux),$e(ex),x=_G(x);Ua=$e(ex);x>>=32;a|(14|$e(ex)<<4)<<4*z4(a));Vv=44+(15&A0(*(V*)&x,I0/2)>>I0%2*4);ns(z4(x),&v))g(nz,Vv=1==tx?ux?~Z:Z:3>tx?cx+Z:5>tx?(V)(u0+ux):(V)(U0+_G(x));L(tx,f,v))
f(vk,Q(x)it=a(xx,ta);P(!t,x)n(Ua=xU;P(t!=ta,x))Qz(2!=Lx)_T(t,_u))f(kv,_K(a_(i,x)))
...
F(mm,QZ)F(No,QZ)F(Dt,QZ)f(dt,QZ)f(ff,P(tx?:Tx,uu(x))QZ)G(NN,Qr(!ta)P(tx,nz(ua,x))Qn(ua>nx)_(ln(f?ua:0,f?nx-ua:ua,x)))f(rr,Qz(2!=Tx)_R(2,nx,n(rc=sx[nx-1-i])))f(qr,Qr(1)0)f(ty,t(3,pt?tx:Tx))g(LMS,QZ)
F(N_,Qt(3!=ta)_d(ua,x))
ZU io(u,U),qz(),z_(U*,U),jJ(u,U,U),a2(u,U,U),o3(u,U,U),fd(u,U),os(),ls(),ys(),tk(),p1();
...
ZU fo(u,U);
g(fs,Lx?$3(f,S2(lu,~Z),S2(mu,Z),4>Tx?e(S2(ai,Z)):e(S2(ae,Z))):$3(f,S0(lg,~Z),S0(mg,Z),S0(ag,Z)))
f(sb,ii=nx/64;if(nx%64)xU&=b(nx%64);r(0,N6(r+=nb(xU))))
g(bo,in=sb(x);$3(f,0<n,nx==n,n))g(bs,Qz(2>f)fs(2,t_(2,x)))

f(n_,_R(3,sb(x),u*s=sr;N6(Ua=xU;j(nb(a),*s++=64*i+IU(a)))))

f(Ss,P(!nx,T(0,0))Ua=ln(-1,nx,x);cc=*sa=x_;a=x(a,_(bC(c==O)));a=n_(a);x=_V(a,x);n(Ua=xU;(sa)[--na]=0)x)
f($,px?c1(_P[x]):tx?lx?$a(x):$(v_(x)):$3(Tx-1,_T(2,48+_b(xu)),10>fo(0,x)?_N(48+O):x,f1($,x)))
...
G(jJ,ii=ta?:Ta;ij=tx?:Tx;P(i*j,it=m(4>f?2:4==f?4:0,m(i,j));a=t>i?t_(t,a):a;x=t>j?t_(t,x):x;tx?ta?5>tx?aa(f,a,x):_v(jJ(f,a,v_(x))):2==f||4==f||2>f-7?QZ:jJ(f,x,a):__(aA(f,a,x)))e2(f,a,x))
g(bp,Un=0;_R(1,nx,N6(Ua=xU;n|=a<<1;rU=f?$6(f-5,a&n,a|n,~a&n,a&~n,0,a^n):n;n=a>>63)))g(fp,P(f,Ua=xx;Ur=x-(1<<Lx);Un=RH;RH=UX[-1];x=a0(a,aA(f,x,r));RH=n;x)a0(0,ln(-1,nx,x)))
...
G(i3,$3(f,P(2==Tx,x=vc(0,x);_((ua?_1:_0)(sx)))QZ,io(a,x),QZ))G(c3,$3(f,QZ,Qr(Tx)sS(a,x),Qr(!Tx)Ss(vc(a,x))))
Zf(g,ff,qr,qr,qr,qr,no,dt,gi,qr,qr,gE,_t,ge,v_,$)Zf(G,ff,rr,qr,qr,og,no,dt,n_,_v,ou,_n,_t,or,v_,$)g(tz,Ua=ut();i(f,Q(_r(z_(0,x))))_(ti(((ut()-a)/(A?2.4e3:2.4e6)-f/2e5))))
g(a1,P(20>f,Qr(px&&17>f-1)$6(f,x,3==tx?++x:jJ(1,tc(1),x),3==tx?--x:jJ(1,tc(-1),x),jJ(3,r_(x),x),j4(-1,x),(tx?g:G)[f-5](x)))LMS(f-20,x))
ZF(J,No,Dt,N_,0,0,0,0,_C,C_,0)
